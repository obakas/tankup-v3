    ...
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Last scan: {formatTime(generatedAt)}
          </p>
        </div>
        <button
          className="h-9 rounded-md border border-slate-300 px-3 text-sm font-semibold text-slate-800 transition hover:border-cyan-600 hover:text-cyan-700 disabled:cursor-wait disabled:opacity-60"
      ...
      {!loading && alerts.length > 0 ? (
        <div className="mt-4 flex max-h-[520px] flex-col gap-3 overflow-auto pr-1">
          {alerts.map((alert) => (
            <button
              className="rounded-md border border-slate-200 p-3 text-left transition hover:border-cyan-600 hover:bg-cyan-50 disabled:border-cyan-700 disabled:bg-cyan-50"
